...
var bool bLunging,bChargingNow;
...
simulated function PostBeginPlay()
{
	...
	ChargeDistance = 250000.f+FRand()*4000000.f;
}
...
simulated function PostNetReceive()
{
	if( bChargingNow )
		MovementAnims[0] = 'Run';
	else MovementAnims[0] = RealWalkAnim;
}
...
function RangedAttack(Actor A)
{
	...
	else if( NextLungeTime<Level.TimeSeconds && IsInMeleeRange(A,250.f) )
	{
		NextLungeTime = Level.TimeSeconds+2.f+FRand()*4.f;
		if( FRand()<0.3f )
			return;
		PrepareStillAttack(A);
	...
	else if( !bChargingNow && VSizeSquared(A.Location-Location)<ChargeDistance )
	{
		RoamAtPlayer();
...
{
	return (!bChargingNow && VSizeSquared(A.Location-Location)<300000.f);
}
...
Begin:
	while( Controller!=None && Controller.Target!=None && VSizeSquared(Controller.Target.Location-Location)<810000.f )
	{
		Sleep(0.5);
		MaxDesiredSpeed = 2.9f;
	}
